...
const SupplierInvoicePreviewModal: React.FC<SupplierInvoicePreviewModalProps> = ({ isOpen, onClose, invoice }) => {
    ...
    (async () => {
      try {
        const [org, qrBase64] = await Promise.all([
          ...
          generateQRBase64(`supplier-invoice:${invoice.idFactureFournisseur}`, 200),
        ]);
        if (cancelled) return;
        const branding = {
        ...
        setHtml(generateSupplierInvoiceHTML(invoice, branding, qrBase64));
      } catch (err) {
        console.error('Failed to build supplier invoice preview', err);
...
export default SupplierInvoicePreviewModal;

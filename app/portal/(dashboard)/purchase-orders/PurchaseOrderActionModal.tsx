...
interface PurchaseOrderActionModalProps {
  isOpen: boolean;
  onClose: () => void;
  order: any;
...
const PurchaseOrderActionModal: React.FC<PurchaseOrderActionModalProps> = ({ isOpen, onClose, order, onStatusChange }) => {
    ...
    (async () => {
      try {
        const [org, qrBase64] = await Promise.all([
          ...
          generateQRBase64(`purchase-order:${order.idBonAchat}`, 200),
        ]);
        if (cancelled) return;
        const branding = {
        ...
        setHtml(generatePurchaseOrderHTML(order, branding, qrBase64));
      } catch (err) {
        console.error('Failed to build purchase order preview', err);
  ...
  const isPending = order.status === 'ENVOYE';

  const handlePrint = async () => {
    setIsPrinting(true);
    try {
      const ok = await sendPrintRequest(html);
      if (ok) toast.success('Purchase order sent to printer successfully!');
  ...
  const handleAccept = async () => {
    ...
    try {
      await PortalApi.acceptPurchaseOrder(order.idBonAchat);
      onStatusChange(order.idBonAchat, 'ACCEPTE');
      toast.success('Purchase order accepted.');
      onClose();
    } catch (err: any) {
      toast.error(err?.message || 'Failed to accept the purchase order.');
    } finally {
  ...
  const handleReject = async () => {
    setIsSubmitting(true);
    try {
      await PortalApi.rejectPurchaseOrder(order.idBonAchat);
      onStatusChange(order.idBonAchat, 'REJETE');
      toast.success('Purchase order rejected.');
      onClose();
    } catch (err: any) {
      toast.error(err?.message || 'Failed to reject the purchase order.');
    } finally {
      setIsSubmitting(false);
    }
  };
      ...
      <div className="bg-slate-50 w-full max-w-4xl max-h-[95vh] rounded-[2rem] overflow-hidden flex flex-col shadow-2xl border border-white/20">
        <div className="bg-white px-8 py-5 border-b border-slate-200 flex justify-between items-center">
          <p className="text-sm font-black uppercase tracking-widest text-slate-700">
            Purchase Order {order.numeroBonAchat}
          </p>
...
export default PurchaseOrderActionModal;

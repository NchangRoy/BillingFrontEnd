  ...
  return (
    <div className='max-w-7xl mx-auto p-6 lg:p-10 flex flex-col gap-8 bg-secondary-super-light/20 min-h-screen'>
      ...
      <div className='flex flex-col md:flex-row md:items-center justify-between gap-6'>
        <div>
          <h1 className='text-secondary text-4xl font-black tracking-tight'>Suppliers</h1>
          <p className='text-gray-500 mt-1 font-medium'>Manage your vendors and service providers</p>
        </div>
      ...
      <div className="p-6 bg-white rounded-2xl shadow-sm border border-secondary-light/20 flex flex-col md:flex-row md:items-center gap-6">
        ...
        <div className="flex gap-3 flex-wrap items-center flex-1">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl border transition-all font-bold text-[11px] uppercase tracking-wider ${
              activeOnly 
              ? "border-secondary-mid bg-secondary-super-light text-secondary-mid shadow-sm" 
              : "border-secondary-super-light text-gray-500 hover:border-secondary-light bg-white"
            }`}
          >
            <span className={activeOnly ? "text-secondary-mid" : "text-gray-300"}>●</span>
          ...
          <div className="relative">
            ...
            >
              <span>{selectedType ? `Type: ${selectedType}` : "Supplier Type"}</span>
            ...
            {showTypeMenu && (
              <div className="absolute top-full left-0 mt-2 bg-white border border-secondary-light/20 rounded-xl shadow-xl z-20 flex flex-col min-w-[220px] overflow-hidden animate-in fade-in slide-in-from-top-1">
                ...
                {Object.values(FournisseurResponse.typeFournisseur).map((type) => (
                  <button
                    key={type}
          ...
          {(activeOnly || selectedType) && (
            <button 
              onClick={() => { setActiveOnly(false); setSelectedType(null); }}
              className='ml-auto text-[10px] font-black uppercase tracking-widest text-red-400 hover:text-red-600 transition-colors'
            >
              Reset Filters
            </button>
          )}
      ...
      <div className='bg-white rounded-2xl shadow-sm border border-secondary-light/20 overflow-hidden'>
        ...
        {filteredSuppliers.length === 0 && (
          <div className='p-24 text-center bg-secondary-super-light/10'>
            ...
            <p className='text-gray-400 font-bold uppercase tracking-widest text-xs'>No matching suppliers found</p>
          </div>
        )}
      </div>
...
// --- MOCK DATA ---
const MOCK_SUPPLIERS: FournisseurResponse[] = [
  {
    idFournisseur: "1",
    username: "JohnDoe",
    typeFournisseur: FournisseurResponse.typeFournisseur.PARTICULIER,
  ...
  },
  {
    idFournisseur: "2",
    username: "Acme Corp",
    typeFournisseur: FournisseurResponse.typeFournisseur.ENTREPRISE,

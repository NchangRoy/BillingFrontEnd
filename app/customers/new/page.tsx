...
import { ClientCreateRequest } from '@/src/api';
import ArrowBackIcon from "@mui/icons-material/ArrowBack";

interface Props {
  initialData?: Partial<ClientCreateRequest>;
  onSubmit: (content: ClientCreateRequest) => void;
}
...
const CustomerFormPage = ({ initialData, onSubmit }: Props) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload: ClientCreateRequest = {
      ...
      typeClient: raw.typeClient as ClientCreateRequest.typeClient,
      raisonSociale: raw.raisonSociale as string || undefined,
      numeroTva: raw.numeroTva as string || undefined,
      codeClient: raw.codeClient as string || undefined,
      ...
      <div className='flex items-center gap-4 mb-6'>
        ...
        <div>
          ...
          <p className='text-xs text-gray-400 mt-1 font-medium'>
            {isEditing ? `Modifying profile: ${initialData.username}` : 'Create a new customer profile'}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className='bg-white rounded-xl shadow-sm border border-gray-100 p-6 lg:p-8'>
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-4'>
          ...
          <div className='md:col-span-2 lg:col-span-3 border-b border-gray-50 pb-1 mb-1'>
            <h3 className='text-secondary-mid text-[10px] font-black uppercase tracking-widest'>1. Account Identity</h3>
          </div>

          <div>
            <label className={labelStyles}>Username *</label>
            <input required type="text" name='username' defaultValue={initialData?.username} className={inputStyles} placeholder="e.g. jdoe_01" />
          </div>
          ...
          <div>
            ...
            <select required name="typeClient" defaultValue={initialData?.typeClient || ""} className={inputStyles}>
              <option value="" disabled>Select...</option>
              {Object.values(ClientCreateRequest.typeClient).map(type => (
          ...
          <div>
            ...
            <input type="email" name='email' defaultValue={initialData?.email} className={inputStyles} placeholder="name@company.com" />
          </div>
          ...
          <div className='md:col-span-2 lg:col-span-3'>
            ...
            <input required name='adresse' defaultValue={initialData?.adresse} className={inputStyles} placeholder="Street, City, Zip" />
          </div>
          ...
          <div className='flex gap-6 mt-2 md:col-span-3'>
            <label className='flex items-center gap-2 cursor-pointer group'>
              ...
              <span className="text-xs font-bold text-gray-500 group-hover:text-secondary-mid transition-colors">Active Account</span>
            </label>
            <label className='flex items-center gap-2 cursor-pointer group'>
        ...
        <div className='mt-8 flex gap-3 justify-end border-t border-gray-50 pt-6'>
          ...
          >
            {isEditing ? 'Update Profile' : 'Save Customer'}
          </button>
        </div>
      </form>
    </div>

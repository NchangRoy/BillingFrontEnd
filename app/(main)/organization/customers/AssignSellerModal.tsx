...
const AssignSellerModal = ({ isOpen, customer, onClose }: Props) => {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      await CustomerAssignmentsService.assign({
        clientId: customer.idClient,
        sellerId: selectedSellerId,
        sellerName: seller?.username,
        organizationId: org.organizationId,
      });
      toast.success(`${customer.raisonSociale || customer.username} assigned to ${seller?.username}.`);
      onClose(true);
    } catch (error) {

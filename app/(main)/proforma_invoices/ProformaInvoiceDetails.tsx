...
const ProformaInvoiceDetails = ({ client, ProformaInvoice, setProformaInvoice }: Props) => {
  ...
  useEffect(() => {
    if (!ProformaInvoice?.lignesDevis?.length) return;
  ...
  }, [ProformaInvoice?.applyVat, client?.ntva]);
  ...
  useEffect(() => {
    ...
    const totalLineDiscounts = (ProformaInvoice?.lignesDevis || []).reduce((acc, line) => acc + (line.remiseMontant || 0), 0);
    ...
    const hasChanged = 
      ...
      Math.round(totalRemiseMontant) !== Math.round(ProformaInvoice.remiseGlobaleMontant || 0);
  ...
  <div className="grid grid-cols-12 gap-6 mt-6 pt-6 border-t border-gray-50">
    ...
    <div className="col-span-12 md:col-span-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-8">
        <SummaryCard title="Total HT" value={ProformaInvoice?.montantHT} icon={<Calculator />} variant="default" />
        <SummaryCard title="TVA (19.25%)" value={ProformaInvoice?.montantTVA} icon={<Tag />} variant="emerald" />
        <SummaryCard title="Total TTC" value={ProformaInvoice?.montantTTC} icon={<Receipt />} variant="default" />
        <SummaryCard title="Total Discount" value={ProformaInvoice?.remiseGlobaleMontant} icon={<Tag />} variant="accent" />
        <SummaryCard title="Final Total" value={ProformaInvoice?.finalAmount} icon={<Receipt />} variant="dark" />
      </div>
    </div>
  );
}

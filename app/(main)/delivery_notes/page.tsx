...
import { Pencil, Trash2, MoreVertical, Printer, Truck, PackageCheck, Clock, XCircle, CheckCircle2 } from "lucide-react";
...
// Mapping display labels to the actual keys in DeliveryNoteResponse
const columns = {
  "DN Number": "deliveryNoteNumber",
  "Recipient": "recipientName",      // Changed from receiverName
  "City": "recipientCity",          // Added for better logistics overview
  "Delivery Date": "deliveryDate",
  "Status": "etat",
  "Total Amount": "totalAmount",
  "Ref SO": "SaleOrderNumber"        // Changed from purchaseOrderNumber
}
...
const DeliveryNotes = () => {
  ...
  const actionOptions = [
    ...
    { 
      ...
      action: (n: DeliveryNoteResponse) => console.log('Confirming Delivery:', n.deliveryNoteNumber),
    ...
    { 
      ...
      icon: <Trash2 size={14} />, 
      action: (n: DeliveryNoteResponse) => {
        setDeliveryNotes(prev => prev.filter(item => item.idDN !== n.idDN));
      },
      color: "text-red-600" 
    },
  ];
  ...
  return (
    <div className='max-w-7xl mx-auto p-6 lg:p-10 flex flex-col gap-8 bg-secondary-super-light/20 min-h-screen'>
      ...
      <div className="p-4 bg-white rounded-2xl shadow-sm border border-secondary-light/20 flex items-center gap-4">
        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 border-r pr-4">Delivery Status</span>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              {filteredNotes.map((note) => (
                <tr key={note.idDN} className="group hover:bg-secondary-mid/[0.01] transition-colors">
                    ...
                    <td key={index} className="px-6 py-4 text-gray-600 font-medium whitespace-nowrap">
                      {value === 'etat' ? (
                        ...
                        <span className="text-xs font-bold text-gray-500">
                           {note.deliveryDate ? new Date(note.deliveryDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : '---'}
                        </span>
                      ) : value === 'SaleOrderNumber' ? (
                        <span className="text-[10px] font-bold bg-secondary-super-light text-secondary-mid px-2 py-1 rounded-lg">
                          {note.SaleOrderNumber || "Direct DN"}
                        </span>
                      ) : (
                        (note as any)[value] || "—"
                      )}
                  ...
                  <td className="px-6 py-4 text-right relative">
                    <button 
                      onClick={() => setActiveMenuId(activeMenuId === note.idDN ? null : (note.idDN ?? null))}
        ...
        {filteredNotes.length === 0 && (
          <div className='py-24 text-center flex flex-col items-center justify-center'>
            <Truck className="text-gray-100 mb-4" size={64} />
            <p className='text-gray-400 font-black uppercase tracking-[0.2em] text-[10px]'>No shipment records found</p>
          </div>
        )}
      </div>
